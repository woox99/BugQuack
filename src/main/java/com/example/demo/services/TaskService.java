...
public class TaskService {
    ...
    public Task createTask(Task task) {
    	...
    	// Capitalize the first letter of the instructions
        String instructions = task.getInstructions();
        instructions = Character.toUpperCase(instructions.charAt(0)) + instructions.substring(1);
        task.setInstructions(instructions);
        
        // create updatedAtString
    	SimpleDateFormat dateFormat = new SimpleDateFormat("MMM d");
        SimpleDateFormat timeFormat = new SimpleDateFormat("h:mma"); 
        task.setUpdatedAtString("on " + dateFormat.format(now) + " at " + timeFormat.format(now));
